...
router.post("/create-shop",upload.single('avatar') ,catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      cloudinary.uploader.upload_stream({folder: "shop"}, (error, result) => {
...
router.put(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (avatar && existsSeller) {
      
        if (existsSeller?.avatar?.public_id) {
        
          await cloudinary.uploader.destroy(existsSeller?.avatar?.public_id);
        ...
        res.status(200).json({ success: true, existsSeller });
      }
